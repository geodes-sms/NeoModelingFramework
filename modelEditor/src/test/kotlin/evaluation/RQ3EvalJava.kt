...
class RQ3EvalJava {
    ...
    @Test
    fun loadEvalData() {
        val rootDir = File("../Evaluation/dataset/models-reverse")
        ...
        for (subfolder in subfolders) {
            ...
            val (filteredXmiFiles, otherXmiFiles) = xmiFiles.partition { file ->
                val name = file.name
                name.contains("eclipseModel-all")
            }
    ...
    fun getFile(operationName: String, metamodel: String): File {
        val resFile = File(
            "../Evaluation/results/RQ3Models-reverse/$metamodel/${operationName}_run_$evalCount.csv"
        )
