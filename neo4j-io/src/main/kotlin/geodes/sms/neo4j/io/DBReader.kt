...
class DBReader(val driver: Driver) {
    ...
    inline fun <R> findNodesWithCounts(
        limit: Int = 100,
        crossinline mapFunction: (NodeResult) -> R
    ): List<R> {
        val params = MapValue(mapOf("limit" to IntegerValue(limit.toLong())))
            ...
            val res = tx.run(
                Query(
                    "MATCH (node)" +
                            ...
                            " WITH node, collect(CASE WHEN rType IS NOT NULL THEN [rType, count] ELSE NULL END) AS pairs" +
                            " RETURN ID(node) AS id, labels(node)[0] AS label," +
                            " apoc.map.fromPairs([p IN pairs WHERE p IS NOT NULL]) AS count", params
                )
            )
    ...
    fun readNodeProperty(id: Long, propName: String): Value {
        ...
        val propValue = session.readTransaction { tx ->
            val res = tx.run(
                Query(
                    "MATCH (n) WHERE ID(n)=\$id RETURN n[\$property] AS prop",
